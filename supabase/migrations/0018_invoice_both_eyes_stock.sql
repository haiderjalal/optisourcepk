-- ============================================================================
-- Invoice R/L lines against stock received for both eyes.
--
-- The order screen adds a prescription as an R line and an L line, but stock
-- is usually received "Both — not eye-specific" (eye blank). issue_invoice
-- looked for the exact eye, then fell back only to a plain SPH bin with no
-- CYL or ADD — so an R/L line on a CYL or ADD lens found nothing and the
-- invoice was refused as out of stock while the shelf held plenty.
--
-- Lookup is now: exact bin -> same power, eye blank -> plain SPH bin.
-- Same function as 0012 otherwise; same signature, so grants carry over.
-- ============================================================================
